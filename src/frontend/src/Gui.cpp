...
namespace epoch::frontend
{
    ...
    void Gui::newFrame(const unsigned width, const unsigned height, const unsigned framebufferWidth,
                       const unsigned framebufferHeight, const double deltaTime) const
    {
        ImGui::GetIO().DisplaySize =
            ImVec2{static_cast<float>(framebufferWidth), static_cast<float>(framebufferHeight)};
        if (width > 0 && height > 0)
        {
            ImGui::GetIO().DisplayFramebufferScale =
                ImVec2{static_cast<float>(framebufferWidth) / static_cast<float>(width),
    ...
    void Gui::render() const
    {
        ...
        for (int n = 0; n < drawData->CmdListsCount; n++)
        {
            ...
            const GLsizeiptr vertexBufferSize =
                static_cast<GLsizeiptr>(cmdList->VtxBuffer.Size) * static_cast<int>(sizeof(ImDrawVert));
            const GLsizeiptr indexBufferSize =
                static_cast<GLsizeiptr>(cmdList->IdxBuffer.Size) * static_cast<int>(sizeof(ImDrawIdx));
    ...
    void Gui::charEvent(const unsigned int c) const { ImGui::GetIO().AddInputCharacter(c); }

    void Gui::focusEvent(const bool focused) const { ImGui::GetIO().AddFocusEvent(focused); }
    ...
    {
        if (button >= 0 && button < ImGuiMouseButton_COUNT) ImGui::GetIO().AddMouseButtonEvent(button, down);
    }

    void Gui::mouseWheelEvent(const float x, const float y) const { ImGui::GetIO().AddMouseWheelEvent(x, y); }
